...
let currentRadius = 5;
let currentBlur = 5;
let currentMaxZoom = 15;
let currentMaxIntense = 1.0;
let currentMinOpacity = 0.05;
...
async function loadAndRenderTracks() {
  ...
  await myHeatmap.addLayer(window.heatLayer);
}
...
document.getElementById('radiusInput').addEventListener('input', function () {
  ...
  window.heatLayer.setOptions({radius: currentRadius});
});

document.getElementById('blurInput').addEventListener('input', function () {
  currentBlur = parseInt(this.value, 10);
  window.heatLayer.setOptions({blur: currentBlur});
});

document.getElementById('maxzoomInput').addEventListener('input', function () {
  currentMaxZoom = parseInt(this.value, 10);
  window.heatLayer.setOptions({maxZoom: currentMaxZoom});
});

document.getElementById('maxintenseInput').addEventListener('input', function () {
  currentMaxIntense = parseFloat(this.value);
  window.heatLayer.setOptions({max: currentMaxIntense});
});

document.getElementById('minopacityInput').addEventListener('input', function () {
  currentMinOpacity = parseFloat(this.value);
  window.heatLayer.setOptions({minOpacity: currentMinOpacity});
});
